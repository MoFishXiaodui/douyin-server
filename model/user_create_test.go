...
func TestCreate(t *testing.T) {
	...
	user := User{UserName: "刀哥", Password: "123", Token: "gg"}
	...
	// 重复创建
	user2 := User{UserName: "刀哥", Password: "77777"}
	_, err = NewUserDaoInstance().Create(user2)
	if err == nil {
		t.Errorf("something wrong in the Create, err:%v", err)
	}

	user3 := User{UserName: "小刀", Password: "123"}
	_, err = NewUserDaoInstance().Create(user3)
	if err != nil {
		t.Errorf("something wrong in the Create, err:%v", err)
	}

	user4 := User{UserName: "baobao"}
	_, err = NewUserDaoInstance().Create(user4)
	if err == nil {
		t.Errorf("something wrong in the Create, err:%v", err)
	}

	user5 := User{UserName: "bao", Password: "asdf46dsa4f6sa1g3sda1g32as1d1xc31@@v131gdf8564wer6q3w46qa"}
